# essense-flow — release history (pre-CHANGELOG entries, verbatim)

These are the entries older than the five most recent, moved VERBATIM out of
`plugins/essense-flow/RELEASE-NOTES.md` when it became
`plugins/essense-flow/CHANGELOG.md` (2026-09-11). Nothing was edited or summarised — the
...
- Drift-6/7/8/9 audit substance (closure-plan SPEC DD-4) — still owed by future increment (unchanged from v0.13.1 closure-reopening's "Scope NOT reopened" list). Drift-6 firing in wild (per S10.8 evidence on the Scalable Crowd reference project) is not closed by F1; F1 closes BS-4 asymmetry only, not the unknown-key-accumulation surface that drift-6 audit would catch.
...
Hotfix per 2026-05-16 closure-reopening decision in `redesign/06-decisions.md`. Surfaces from real-project failures in the Scalable Crowd reference project using cached `0.13.0`; root causes verified against cached source line-by-line before any code touch. Three concrete fixes + baseline test green; no new public API; additive `sprint_iteration` state field.
