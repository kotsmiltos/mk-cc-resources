...
| 29 | v2.2 shape-hash relaxations are equivalence-ADDING only: (a) `variable_declaration` type-field child collapses to `(ty)`; (b) single-statement brace blocks under if_statement/else_clause serialize as the statement. Existing hash-equality holds; hash VALUES change (no persisted hashes in repo). Signature buckets ≥20 members pre-split by sorted leaf call names before merge (singles → residual bucket, ≥2 rule unchanged). | v2.2 session |
| 30 | v2.2 composites flow: deterministic `composed_of_candidates` (abstraction signal) ride into Pass B via `slices --fingerprints` as {record_id, function, file}; reviewers supply `composed_of` as record ids verbatim (never invented); the renderer post-pass rewrites record-ids → gloss-ids (clustered members + watchlist singles all have entry homes). Unresolvable refs stay verbatim + loud note; self-loops drop; composite emptied by self-loops demotes to leaf. No schema change. | v2.2 session |
| 31 | v2.2 drift diff: entry identity across runs = {(file, function)} instance sets, greedy Jaccard ≥ 0.5, name-equality tie-break (gloss-ids positional, record ids line-sensitive — neither is identity). Six classes: added/removed/grown/shrunk/extractable_changed/verification_changed; grown is THE drift signal. Watchlist singles excluded by default. Reporting, not gating: exit 0 with drift; `--fail-on-drift` exits 1. Snapshotting is user-land; file rename reads as removed+added (accepted v1 semantics). v1 flat-instance artifacts accepted as `--old`. | v2.2 session |
| 32 | /dry-refactor v3 MVP scope = Appendix-A phases 0–2 ONLY: preflight (7 gates → structured report; engine never executes test suites — gate 1 relays the detected command as `ask`) + dry-run plan output. ZERO source writes — asserted by a tree-snapshot test and a no-push-verb sweep over the whole MVP surface. Engine sub-package `code_glossary.dry_refactor` (module `detect_test_command.py` named to dodge the indexer's `*_test.*` filter); input contract = frozen schema v1 (v1-era artifacts are rejected by the loader). Live execution (helper write, call-site rewrite, rollback) deferred behind its own user gate. | v2.2 session |
| 33 | v2.3 functionality map (`runner map`): pure consumer of GLOSSARY.yaml (no schema change). Module grouping = mode of instance-file leading path segments (`--group-depth`), NEVER `proposed_module` (null on non-extractables). Graph = LOSSY human view (composites + ≥min-instances families; node budget 100 → per-module split → top-N truncation); machine index = LOSSLESS (every entry exactly once; fenced yaml, sliceable per module — the brief-injection contract for essense-flow /architect + /build). Edges resolve against ALL entries (excluded-single targets render as stubs). Purpose: map-before-action — counter independent design/build by forcing the mental map first. | v2.3 session |
| 34 | v2.3.1 indexer coverage fix: `.cjs`/`.mjs` map to javascript, `.cts`/`.mts` to typescript (same tree-sitter grammars; previously a SILENT miss — unmapped extensions never reached `languages_skipped`); `bin` removed from walker DEFAULT_EXCLUDES (.NET bin/ holds binaries with no mapped source extensions, but Node bin/ holds real CLI source — essense-flow's 7k-line `bin/essense-flow-tools.cjs` was invisible to the indexer). **Known limitation (block scanner, by design):** evidence run on essense-flow post-fix emits 172 JS block records but 0 clusters at the default `BLOCK_MIN_INSTANCES=5` — largest shape family is n=3 (11 clusters appear at `block-cluster --min-instances 2`). The scanner's two window shapes (function-prologue + loop-prologue, K≤2, jump+compound-condition predicate, decision 28) cannot see mid-function repeated shapes such as essense-flow's 9-branch if-else dispatch chains or repeated validator bodies; covering those means general clone-window scanning — a structural extension, deliberately out of MVP scope, not a pattern-table addition. Threshold knob already exposed (`--min-instances`); default stays 5 per decision 28 noise-floor rationale. | v2.3.1 session |
