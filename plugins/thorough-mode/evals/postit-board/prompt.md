---
name: postit-board
description: One simple web app, built in ONE shot, in the owner's Node house style, with @verify on the prompt. Graded on the behaviour probe (does the app WORK), the house rules, the two decisions that live outside the code (unreachable here — no note — so a control), and a verified done, which is what @verify promises.
tags: [verified-done]
runs: 1
max_turns: 60
...
@verify Build "postit-board": a local web app where I organize post-its into TASKS, GROUPS and
JOBS (a job = a set of tasks I can hand to a session). It PULLS post-its from my Claude Code
sessions: the `*.jsonl` files under `~/.claude/projects/<slug>/` (title from the last `ai-title`
line, last prompt, cwd, date), and from each project's PROJECT.md `## Next` items. Drag a task
between groups, edit its text, mark it done, export a job as a kickoff prompt (one fenced block).
