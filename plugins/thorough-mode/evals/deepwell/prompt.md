---
name: deepwell
description: A small survival-automation sim (no quests, progress at your own pace), built in ONE shot, with @verify on the prompt. Graded on the behaviour probe (does the sim WORK), the house rules, the two decisions that live outside the code (unreachable here — no note — so a control), and a verified done, which is what @verify promises.
tags: [verified-done]
runs: 3
max_turns: 100
...
---
@verify OWNER ASKED (verbatim): "something about a survival game where you just progress without having
