---
name: deepwell
description: A small survival-automation sim (no quests, progress at your own pace), built in ONE shot. Two decisions (lamp burn time only in config.json; ONE random source createRng) live ONLY in .claude/kb/extracted, hints opted in — WITH kb the pull hint points at the entry. Scored by a behaviour probe (probe.json) that drives the headless sim, plus the decisions on disk.
