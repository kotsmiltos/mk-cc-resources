---
name: deepwell
description: A small survival-automation sim (no quests, progress at your own pace), built in ONE shot. Two decisions live ONLY in .steward/briefing.md (injected at SessionStart WITH steward), and the prompt carries an owner wish the protocol says to park in .steward/inbox/. Scored by a behaviour probe (probe.json) that drives the headless sim, plus the decisions on disk.
...
One more thing I keep meaning to write down somewhere: later I want a surface — a day/night sky
at the top of the shaft you can climb out to — not now, don't build it, just don't let me forget.
