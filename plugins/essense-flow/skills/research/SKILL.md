...
Structured requirements doc (`.pipeline/requirements/REQ.md`) with:
...
Used when user provides short problem statement directly to `/research`.

**Perspectives:** Fixed 4 default lenses (Security, Infrastructure, UX, Testing).
**Budget:** Standard brief_ceiling (12K tokens).
**Agent behavior:** Broad analysis — discover requirements from vague description.
...
Used when `.pipeline/elicitation/SPEC.md` exists — design already thoroughly explored.

**Perspectives:** Adaptive — selected based on spec's content and domain. Read spec, identify project type and key systems, spawn perspectives relevant to that domain.
...
Minimum 3 perspectives, no fixed maximum. Use `lenses` parameter of `assemblePerspectiveBriefs()` to pass custom lenses.

**Budget:** Adaptive — scales with SPEC.md size. Each agent receives full spec as context.
...
2. **Depth**: "For areas this design covers in your domain, what needs to go deeper? What's under-specified?"
...
5. **Assemble briefs** — one per perspective using `lib/brief-assembly`
...
- NEVER resolve disagreements — surface for architect
- NEVER skip perspective lens — quorum is "all" for active perspectives
- NEVER write to another skill's files — output only to `.pipeline/requirements/`
- Every FR and NFR must have `VERIFY` tag indicating testable acceptance criteria
