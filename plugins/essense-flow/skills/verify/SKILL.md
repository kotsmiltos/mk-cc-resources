...
7. `essense-flow-tools state-set-verify-completed --value <iso8601>` — stamp the verify exit timestamp. As of v0.20.0 (issue #3) this also mirrors VERIFICATION-REPORT.md's frontmatter summary (`items_total`, `implemented`, `partial`, `missing`, `drift`, `confirmed_gaps`, `completion_status`) into the `state.verify` cache block so the cache reflects this audit, not a prior round (fail-soft if the report is absent — write it in step 6 first). NOT a substitute for `state-set-phase`.
...
## Skip-IFF rule for verifier dispatch

This SKILL.md is the authoritative substance source; the block below is the canonical rule text.

## Verifier-dispatch Skip-IFF rule

Phase-exit gates are objective and machine-checked: skipping sub-agent dispatch is allowed only under the explicit conditions below, counted from the output artifact — there is no bypass flag, and "the environment can't do it" is not an admissible skip cause. The default discipline: verify-skill per-item verifier dispatch count ≥ items_total (where items_total = count of completion records in the sprint under verify). Master verify MAY skip per-item verifier dispatch ONLY IFF EITHER:

1. **items_total == 0** — the sprint has zero completion records to verify (vacuous case; no verifier to dispatch).

OR

2. **rule-allowed-substance-quote cited** — VERIFICATION-REPORT.md frontmatter or master synthesize note carries a verbatim rule quote from this SKILL.md's Skip-IFF section or from a closed design decision authorizing the condensed-verify path for this sprint. Citation MUST include the rule-quote text + source decision ID.
...
**Predicate enforcement.** evalDispatchPredicate (`bin/essense-flow-tools.cjs:2013`) recognizes phrase 'with sufficient verifier dispatch' (`DISPATCH_PHRASES` entry at `bin/essense-flow-tools.cjs:1965`) → counts vs threshold via cursorState.alignment_lens_dispatches_per_round.verifier bucket; the rule-allowed-skip path is honored when a verbatim rule-quote citation is present.

**Drift detection.** drift-9 substantive check scans VERIFICATION-REPORT.md frontmatter post-hoc.

Verifiable check: spawn verify skill on fixture sprint with items_total=8 + verifier_dispatches_total=0 + no rule-quote → state-set-phase verifying→complete refused with EXIT_ALIGNMENT_DRIFT (19) + diagnostic naming the failed dispatch-sufficiency rule by its sourceKey.
...
- Per **No-Resource-Caps** (`references/principles.md`): no cap on items. The spec defines the item count.
...
- `lib/dispatch.js` — DEPRECATED for verify (replaced by registered `essense-flow-extractor` + `essense-flow-item-verifier` agent dispatch via the Agent tool with `subagent_type`). Kept in tree for unmigrated skills.
...
**The exact CLI op sequence** for the verifying→complete transition:
...
`essense-flow-tools init verify` (verbatim). Steps emit one at a time so
consumed steps fall out of context; the cursor advances only on explicit
`step-advance`. Bodies above remain the source-of-truth; the parser
(lib/cursor-schema.cjs `parseSkillStepsFromMarkdown`) slices emission
windows at these headings — the heading shape is the parser contract.

## 1. extract-spec-decisions

Step 1 of 6 for the verify skill (ordered_steps anchor).
...
Step 2 of 6 for the verify skill (ordered_steps anchor).
...
Step 3 of 6 for the verify skill (ordered_steps anchor).
...
Step 4 of 6 for the verify skill (ordered_steps anchor).
...
Step 5 of 6 for the verify skill (ordered_steps anchor).
...
Step 6 of 6 for the verify skill (ordered_steps anchor).
