...
- `coupling` — does each unit stay decoupled, or reach across a boundary into another unit's internals? The enforcement arm of the lead code convention ("build decoupled" — `references/code-conventions.md`); the only legal cross-unit dependency is a declared contract (`exposes`/`consumes` in the task specs). Flags cross-boundary reach-ins (calling a sibling's private surface), concrete-instead-of-contract dependencies, undeclared cross-unit dependencies, shared mutable state across units, and circular deps — each with `file_path:line_number` + verbatim quote. **Dispatch this lens whenever the sprint wrote >1 unit or any unit consumes another's output.** A confirmed cross-boundary reach-in or circular dependency is normally `critical` → blocks the sprint at the deterministic gate (decoupling is a gate, not advice).
...
- **spec-drift** — does the implementation match the spec claim at the cited locator?
- **functional-testing** — read the tests for what they actually verify
- **coupling** — does each unit stay decoupled, or reach across a boundary into another unit's internals? Trace the real import/call/field-access edges against the `exposes`/`consumes` contracts; flag cross-boundary reach-ins, concrete-instead-of-contract deps, undeclared cross-unit deps, shared mutable state across units, circular deps. Enforces the lead code convention (`references/code-conventions.md` "build decoupled"); confirmed reach-ins are `critical` and block the sprint. Dispatch whenever the sprint wrote >1 unit.
