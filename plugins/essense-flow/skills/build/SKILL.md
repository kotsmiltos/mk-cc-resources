...
1. **Dispatch.** For every task in the wave, in parallel:
   ...
   - Append two context blocks after the task-spec fields, clearly marked as **context, not contract** (the task spec remains the only contract):
     ...
     - **`NEIGHBORS IN THIS WAVE`** (when the wave has >1 task): for every OTHER task in the SAME wave, show its **contract, not its goal** — `- <task_id> exposes: <each entry of that task's `exposes` block, ≤120 chars>`. A sibling with no `exposes` block falls back to `- <task_id>: (no declared contract) <goal first sentence>`, explicitly tagged so the agent knows there is no surface to bind to yet. **Why the contract and not the goal:** the agents write in parallel and blind; showing a sibling's *goal* invites coupling to what it does, showing its *interface* lets a consumer integrate against the shape and a provider avoid duplicating it — neighbor-awareness that serves decoupling instead of eroding it. Agents bind ONLY to a neighbor's `exposes`; reaching past it is a coupling defect the review `coupling` lens will flag.
   ...
   - Append one reminder line to every brief: a design fork discovered mid-build ("X or Y for different goals?") routes through the generativity protocol (`references/generativity-protocol.md`) — surface it in `surfaced_concerns` per Front-Loaded-Design; never hardcode the one instance you happened to need.
