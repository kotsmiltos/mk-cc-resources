...
essense-flow drives a multi-phase pipeline that spawns sub-agents, reads/writes `.pipeline/` artifacts, and edits source files in the project directory. The trust posture is documented here so operators know what the plugin assumes, what it does not check, and where the sharp edges sit at 0.18.
...
- **Malicious or buggy SKILL.md content** — skills cite the canonical Conduct and principles from `references/principles.md` (citation-audited at build time), but the plugin does not cryptographically sign skill bodies. An operator who pulls the marketplace and modifies a SKILL.md locally can change agent behavior. The mitigation is install-time trust of the marketplace source, not run-time validation.
...
- **State-file tampering** — `.pipeline/state.yaml` is a derived cache; the artifacts are authoritative. The CLI ops (`state-set-phase`, `record-task-completion`) are the legitimate writers; every phase-producing SKILL.md closes with the exact invocation so direct writes stay the visible deviation. A hand-edited, deleted, or corrupted cache is recoverable: `state-reconcile` compares the cache against artifact inference and `--apply` rebuilds it from disk (HEAL-LOG audited) — tampering is detectable and reversible, not silently trusted.
...
- **Supply-chain compromise of the marketplace** — install pulls from GitHub. A compromise of the marketplace repo would propagate to every install on next upgrade. Mitigation today: pin to a tagged commit; review the diff before upgrade. 0.18 does not ship signed releases.
...
There is no SLA at 0.18. The plugin is a single-maintainer project; response time depends on availability. Operators who need stronger guarantees should pin to a vendored copy and review diffs before upgrading.

## Mitigations

- **Gated state writes.** `state-set-phase` is the phase advancer — it validates the edge against `transitions.yaml` and checks prerequisite artifacts on disk before moving; `record-task-completion` is the sole writer of completion records. Every phase-producing SKILL.md ends with a closing block naming the legal phase targets and the exact invocation. Master writing `state.yaml` directly is the failure mode the closing block exists to prevent — and `state-reconcile` exists to detect and repair when it happens anyway, because the artifacts, not the cache, are authoritative.
...
- **Dual-record spot-check still advised.** Master re-validates every agent self-report against disk, but the verification covers the claims the spec's completion contract names — it is not a full semantic diff of agent output. Operators running large fan-outs should spot-check `agent_claim` content against `runner_verification` for the first few tasks of any new pipeline.
...
- **Single-maintainer bus factor.** The plugin has one maintainer at 0.18. Security response, bug fix latency, and upgrade cadence are all bottlenecked on that maintainer's availability.
