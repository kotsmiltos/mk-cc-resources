...
9. **Scope check before a long run.** A pasted kickoff is a proposal, not a law: when it is far
   heavier than the goal behind it, say so and confirm before the first hour of dispatches.
...
- **The second face measured the wrong way first.** The morning kickoff ran essense-flow's full
  ...
  stay under `postit-board\.pipeline\` as a record (uncommitted repo). What the
...
- **The single-shot measure, as the owner meant it** (`plugin-eval`, case `postit-board`: build
  ...
  | turn-end | 0.80 | 1.00 | −0.20 | 1265 / 1167 | verified-done 0/1 WITH: the arm hit `max_turns` (61) mid-fix ("Re-reading turned up two real bugs. Fixing both.") and never yielded; the page duty cost it the ending. Page rewritten 1/1 in BOTH arms (the prompt said "read the page first") |
...
- **Kickoff prompts that say "ask nothing"**: keep the phrase, or replace it with "ask once, at
  the scope line"? Default: replace — today's run is the measurement (law 9).
