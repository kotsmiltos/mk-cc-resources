...
`plugins/plugin-toolkit/CLAUDE.md`), plus five repo-level gates run from the toolkit dir:
...
| `node plugins/plugin-toolkit/bin/plugin-eval.js --root .` (1.20.0) | "Is the owner better off WITH it?" — **only when the owner asks for a run**, after telling him how many sessions it starts and what it will cost. Never as a routine step after a ship. | The with/without table, one per plugin that ships `evals/`: `claude plugin eval --ablation with-without` (3 runs/arm, the agent on opus, the judge on sonnet, `--max-cost-usd` per run — checked only before each run starts, so it is not a total cap; report kept local), WITH / W/OUT / Δ / seconds per case plus which of the three wants moved (progress captured · decisions honoured · verified done). Real, paid `claude` sessions on his credential — `--dry-run` first. Cases: `deepwell` in kb, patterns, steward, thorough-mode, turn-end (one each): a bare call starts 30 opus sessions (5 cases × 2 arms × 3 runs); the 2026-09-20 run of exactly that cost about $167. Windows: no Bash in cases; scaffold seeds each cwd from `fixtures/`. Exit 0 always (a report), 2 cannot run. |
