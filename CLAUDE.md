...
| `node plugins/plugin-toolkit/bin/plugin-eval.js --root .` (1.20.0) | "Is the owner better off WITH it?" — after a ship, before retiring a plugin | The with/without table, one per plugin that ships `evals/`: `claude plugin eval --ablation with-without` (3 runs/arm, sonnet both sides, `--max-cost-usd`, report kept local), WITH / W/OUT / Δ / seconds per case plus which of the three wants moved (progress captured · decisions honoured · verified done). Real `claude` children on your credential — `--dry-run` first. Suites: kb, steward, turn-end, thorough-mode (one case each, 2026-09-18). Windows: no Bash in cases; scaffold seeds each cwd from `fixtures/`. Exit 0 always (a report), 2 cannot run. |
